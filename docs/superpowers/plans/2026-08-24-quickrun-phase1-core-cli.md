...
public static string ToYaml(Candidate c, string? name)
{
    ...
    sb.AppendLine("# yaml-language-server: $schema=https://quickrun.org/quickrun.schema.json");
...
6. Load the config, in order: `args.ConfigPath` if given, else `ConfigParser.FindConfigFile(root)`,
   ...
   `"no quickrun.yml, no run script and nothing detectable in <repo> - see https://quickrun.org/docs/config"`.
...
(`https://quickrun.org/` and `/de/`), and a link to `samples/`.
